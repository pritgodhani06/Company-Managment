  ...
  return <AriaSlider {...rest} {...{
    ...
    maxValue
        ...
        return <>
                            ...
                            <span className="absolute top-1/2 h-2 w-full -translate-y-1/2 rounded-full bg-brand-solid" style={{
            left: `${left * 100}%`,
            width: `${width * 100}%`
          }} />
                            {values.map((_, index) => {
            return <AriaSliderThumb key={index} index={index} className={({
              isFocusVisible,
              isDragging
            }) => cx("top-1/2 box-border size-6 cursor-grab rounded-full bg-slider-handle-bg shadow-md ring-2 ring-slider-handle-border ring-inset", isFocusVisible && "outline-2 outline-offset-2 outline-focus-ring", isDragging && "cursor-grabbing")}>
