...
export const TagGroup = ({
  ...
  return <TagGroupContext.Provider value={{
    selectionMode,
    size
  }}>
...
export const Tag = ({
  ...
  const leadingContent = avatarSrc ? <Avatar size="xxs" src={avatarSrc} alt="Avatar" contrastBorder={avatarContrastBorder} /> : dot ? <Dot className={cx("text-fg-success-secondary", dotClassName)} size="sm" /> : null;
  return <AriaTag id={id} isDisabled={isDisabled} textValue={typeof children === "string" ? children : undefined} className={state => cx("flex cursor-default items-center gap-0.75 rounded-md bg-primary text-secondary ring-1 ring-primary ring-inset focus:outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring", styles[context.size].root.base,
  // With avatar
  ...
  // Disabled
  isDisabled && "cursor-not-allowed", typeof className === "function" ? className(state) : className)}>
            {({
      isSelected,
      isDisabled,
      allowsRemoving
                    ...
                    <div className={cx("flex items-center gap-1", styles[context.size].content)}>
                        ...
                        {typeof count === "number" && <span className={cx("flex items-center justify-center rounded-[3px] bg-tertiary text-center", styles[context.size].count)}>
                                {count}
                            </span>}
                    </div>
