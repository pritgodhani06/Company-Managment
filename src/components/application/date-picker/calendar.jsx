...
export const CalendarContextProvider = ({
  ...
  return <AriaCalendarContext.Provider value={{
    value,
    onChange,
    focusedValue,
    onFocusChange
  }}>{children}</AriaCalendarContext.Provider>;
};
const PresetButton = ({
  value,
  children,
  ...
  slot={null} size="md" color="secondary" onClick={handleClick}>
            {children}
        </Button>;
};
            ...
            <AriaCalendar {...props} className={state => cx("flex flex-col gap-3", typeof className === "function" ? className(state) : className)}>
                ...
                <AriaCalendarGrid weekdayStyle="short" className="w-max">
                    <AriaCalendarGridHeader className="border-b-4 border-transparent">
                        {day => <AriaCalendarHeaderCell className="p-0">
