...
export const DateRangePicker = ({
  ...
  const presets = useMemo(() => ({
    ...
    lastWeek: {
      label: "Last week",
      value: {
        start: startOfWeek(now, locale).subtract({
          weeks: 1
        }),
        end: endOfWeek(now, locale).subtract({
          weeks: 1
        })
      }
    },
    ...
    lastMonth: {
      label: "Last month",
      value: {
        start: startOfMonth(now).subtract({
          months: 1
        }),
        end: endOfMonth(now).subtract({
          months: 1
        })
      }
    },
    thisYear: {
      label: "This year",
      value: {
        start: startOfMonth(now.set({
          month: 1
        })),
        end: endOfMonth(now.set({
          month: 12
        }))
      }
    },
  ...
  return <AriaDateRangePicker aria-label="Date range picker" shouldCloseOnSelect={false} {...props} value={value} onChange={setValue}>
            <AriaGroup>
            ...
            <AriaPopover placement="bottom right" offset={8} className={({
      isEntering,
      isExiting
          ...
          close
                            ...
                            <div className="hidden w-38 flex-col gap-0.5 border-r border-solid border-secondary p-3 lg:flex">
                                {Object.values(presets).map(preset => <RangePresetButton key={preset.label} value={preset.value} onClick={() => {
              ...
              setFocusedValue(preset.value.start);
                            ...
                            <div className="flex flex-col">
                                <RangeCalendar focusedValue={focusedValue} onFocusChange={setFocusedValue} highlightedDates={highlightedDates} presets={{
