...
export const RangeCalendarContextProvider = ({
  ...
  return <RangeCalendarContext.Provider value={{
    value,
    onChange,
    focusedValue,
    onFocusChange
  }}>{children}</RangeCalendarContext.Provider>;
};
...
const MobilePresetButton = ({
  ...
  const context = useContext(RangeCalendarStateContext);
  return <Button {...props} slot={null} size="sm" color="link-color" onClick={() => {
...
export const RangeCalendar = ({
  ...
  return <ContextWrapper>
            <AriaRangeCalendar className="flex items-start" visibleDuration={{
      months: isDesktop ? 2 : 1
                ...
                <div className="flex flex-col gap-3 px-6 py-5">
                    ...
                    {!isDesktop && <div className="flex items-center gap-2 md:hidden">
                            <DateInput slot="start" className="flex-1" />
                            <div className="text-md text-quaternary">–</div>
                            <DateInput slot="end" className="flex-1" />
                        </div>}

                    {!isDesktop && presets && <div className="mt-1 flex justify-between gap-3 px-2 md:hidden">
                            {Object.values(presets).map(preset => <MobilePresetButton key={preset.label} value={preset.value}>
                                    {preset.label}
                                </MobilePresetButton>)}
                        </div>}
                    ...
                    <AriaCalendarGrid weekdayStyle="short" className="w-max">
                        <AriaCalendarGridHeader>
                            {day => <AriaCalendarHeaderCell className="border-b-4 border-transparent p-0">
                ...
                {isDesktop && <div className="flex flex-col gap-3 border-l border-secondary px-6 py-5">
                        ...
                        <AriaCalendarGrid weekdayStyle="short" offset={{
          months: 1
        }} className="w-max">
                            <AriaCalendarGridHeader>
                                {day => <AriaCalendarHeaderCell className="border-b-4 border-transparent p-0">
