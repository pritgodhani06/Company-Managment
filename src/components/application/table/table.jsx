...
export const TableRowActionsDropdown = () => <Dropdown.Root>
        <Dropdown.DotsButton />
...
const TableCardHeader = ({
  ...
  return <div className={cx("relative flex flex-col items-start gap-4 border-b border-secondary bg-primary px-4 md:flex-row", size === "sm" ? "py-4 md:px-5" : "py-5 md:px-6", className)}>
            <div className="flex flex-1 flex-col gap-0.5">
                <div className="flex items-center gap-2">
                    <h2 className={cx("font-semibold text-primary", size === "sm" ? "text-md" : "text-lg")}>{title}</h2>
                    {badge ? isValidElement(badge) ? badge : <Badge color="brand" size="sm">
                                {badge}
                            </Badge> : null}
                </div>
...
const TableHeader = ({
  ...
  return <AriaTableHeader {...props} className={state => cx("relative bg-secondary", size === "sm" ? "h-9" : "h-11",
  ...
  bordered && "[&>tr>th]:after:pointer-events-none [&>tr>th]:after:absolute [&>tr>th]:after:inset-x-0 [&>tr>th]:after:bottom-0 [&>tr>th]:after:h-px [&>tr>th]:after:bg-border-secondary [&>tr>th]:focus-visible:after:bg-transparent", typeof className === "function" ? className(state) : className)}>
            {selectionBehavior === "toggle" && <AriaColumn className={cx("relative py-2 pr-0 pl-4", size === "sm" ? "w-9 md:pl-5" : "w-11 md:pl-6")}>
                    {selectionMode === "multiple" && <div className="flex items-start">
...
const TableHead = ({
  ...
  return <AriaColumn {...props} className={state => cx("relative p-0 px-6 py-2 outline-hidden focus-visible:z-1 focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-offset-bg-primary focus-visible:ring-inset", selectionBehavior === "toggle" && "nth-2:pl-3", state.allowsSorting && "cursor-pointer", typeof className === "function" ? className(state) : className)}>
            {state => <AriaGroup className="flex items-center gap-1">
                    ...
                    {state.allowsSorting && (state.sortDirection ? <ArrowDown className={cx("size-3 stroke-[3px] text-fg-quaternary", state.sortDirection === "ascending" && "rotate-180")} /> : <ChevronSelectorVertical size={12} strokeWidth={3} className="text-fg-quaternary" />)}
                </AriaGroup>}
        </AriaColumn>;
};
...
const TableRow = ({
  ...
  return <AriaRow {...props} className={state => cx("relative outline-focus-ring transition-colors after:pointer-events-none hover:bg-secondary focus-visible:outline-2 focus-visible:-outline-offset-2", size === "sm" ? "h-14" : "h-18", highlightSelectedRow && "selected:bg-secondary",
  // Row border—using an "after" pseudo-element to avoid the border taking up space.
  "[&>td]:after:absolute [&>td]:after:inset-x-0 [&>td]:after:bottom-0 [&>td]:after:h-px [&>td]:after:w-full [&>td]:after:bg-border-secondary last:[&>td]:after:hidden [&>td]:focus-visible:after:opacity-0 focus-visible:[&>td]:after:opacity-0", typeof className === "function" ? className(state) : className)}>
            {selectionBehavior === "toggle" && <AriaCell className={cx("relative py-2 pr-0 pl-4", size === "sm" ? "md:pl-5" : "md:pl-6")}>
...
const TableCell = ({
  ...
  return <AriaCell {...props} className={state => cx("relative text-sm text-tertiary outline-focus-ring focus-visible:z-1 focus-visible:outline-2 focus-visible:-outline-offset-2", size === "sm" && "px-5 py-3", size === "md" && "px-6 py-4", selectionBehavior === "toggle" && "nth-2:pl-3", typeof className === "function" ? className(state) : className)}>
            {children}
        </AriaCell>;
};
