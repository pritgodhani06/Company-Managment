...
export const SidebarNavigationSlim = ({
  ...
  const mainSidebar = <aside style={{
    width: MAIN_SIDEBAR_WIDTH
  }} className="group flex h-full max-h-full max-w-full overflow-y-auto bg-primary py-1 pl-1">
            <div className={cx("flex w-auto flex-col justify-between rounded-xl pt-5 ring-1 ring-secondary transition duration-300 ring-inset", hideBorder && !isSecondarySidebarVisible && "ring-transparent")}>
                ...
                <ul className="mt-4 flex flex-col gap-0.5 px-3">
                    {items.map(item => <li key={item.label}>
                            <NavItemButton size="md" current={currentItem.href === item.href} href={item.href} label={item.label || ""} icon={item.icon} onClick={() => setCurrentItem(item)} />
                        </li>)}
                </ul>
                <div className="mt-auto flex flex-col gap-4 px-3 py-5">
                    {footerItems.length > 0 && <ul className="flex flex-col gap-0.5">
                            {footerItems.map(item => <li key={item.label}>
                                    <NavItemButton size="md" current={currentItem.href === item.href} label={item.label || ""} href={item.href} icon={item.icon} onClick={() => setCurrentItem(item)} />
                                </li>)}
                        </ul>}
            ...
            isFocused
                        ...
                        <AriaPopover placement="right bottom" offset={8} crossOffset={6} className={({
            isEntering,
            isExiting
          }) => cx("will-change-transform", isEntering && "duration-300 ease-out animate-in fade-in placement-right:slide-in-from-left-2 placement-top:slide-in-from-bottom-2 placement-bottom:slide-in-from-top-2", isExiting && "duration-150 ease-in animate-out fade-out placement-right:slide-out-to-left-2 placement-top:slide-out-to-bottom-2 placement-bottom:slide-out-to-top-2")}>
                            <NavAccountMenu />
                        </AriaPopover>
                    </AriaDialogTrigger>
                </div>
  ...
  const secondarySidebar = <AnimatePresence initial={false}>
            {isSecondarySidebarVisible && <motion.div initial={{
      width: 0,
      ...
      bounce: 0
        ...
        width: SECONDARY_SIDEBAR_WIDTH
                        ...
                        <ul className="py-2">
                            {currentItem.items?.map(item => <li key={item.label} className="py-0.5">
  ...
  return <>
            ...
            <div className="z-50 hidden lg:fixed lg:inset-y-0 lg:left-0 lg:flex" onPointerEnter={() => setIsHovering(true)} onPointerLeave={() => setIsHovering(false)}>
                {mainSidebar}
                {secondarySidebar}
            </div>
                ...
                <aside className="group flex h-full max-h-full w-full max-w-full flex-col justify-between overflow-y-auto bg-primary pt-4">
                    ...
                    <div className="mt-auto flex flex-col gap-5 px-2 py-4">
                        ...
                        <div className="relative flex items-center gap-3 border-t border-secondary pt-6 pr-8 pl-2">
                            <AvatarLabelGroup status="online" size="md" src="https://www.untitledui.com/images/avatars/olivia-rhye?fm=webp&q=80" title="Olivia Rhye" subtitle="olivia@untitledui.com" />

                            <div className="absolute top-1/2 right-0 -translate-y-1/2">
                                <Button size="sm" color="tertiary" iconLeading={<LogOut01 className="size-5 text-fg-quaternary transition-inherit-all group-hover:text-fg-quaternary_hover" />} className="p-1.5!" />
                            </div>
                        </div>
                    </div>
                </aside>
