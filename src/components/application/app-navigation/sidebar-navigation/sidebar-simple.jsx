...
export const SidebarNavigationSimple = ({
  ...
  const content = <aside style={{
    "--width": `${MAIN_SIDEBAR_WIDTH}px`
  }} className={cx("flex h-full w-full max-w-full flex-col justify-between overflow-auto bg-primary pt-4 lg:w-(--width) lg:pt-6", !hideBorder && "border-secondary md:border-r", className)}>
            ...
            <div className="mt-auto flex flex-col gap-4 px-2 py-4 lg:px-4 lg:py-6">
                {footerItems.length > 0 && <ul className="flex flex-col">
                        {footerItems.map(item => <li key={item.label} className="py-0.5">
